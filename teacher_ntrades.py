...
def train(trainloader, model, optimization, start_epoch, stop_epoch, params,config):    
    ...
    for epoch in range(start_epoch,stop_epoch):
        ...
        avg_loss=0
        correct ,total =0,0
        for i, (x,y,gt) in enumerate(trainloader):
            
            a = time.time()
            x,y = x.cuda(), y.cuda()
            x_adv = pgd.attack4(model,x,y)
            optimizer.zero_grad()
            scores,_ = model.forward(x)
            predicted = torch.argmax(scores,1)
            correct += (predicted==y).sum().item()
            total += predicted.size(0)
            loss = loss_fn(scores,y) + (1.0 / x.size(0)) * criterion_kl(F.log_softmax(model(x_adv)[0], dim=1),F.softmax(model(x)[0], dim=1))
            loss.backward()
            optimizer.step()
            b = time.time()
            t += b-a
...
if __name__=='__main__':
    ...
    optimization = 'Adam'
    params.checkpoint_dir = '%s/%s/teacher/%s_%s' %( SAVE_DIR,params.dataset, params.model, 'ntrades')
