...
def train(trainloader, model, teacher,optimization, start_epoch, stop_epoch, params,config): 
    ...
    for epoch in range(start_epoch,stop_epoch):
        ...
        for i, (x,y,target_mask) in enumerate(trainloader):
            ...
            if params.method == 'bbox_adv':
                x_ = attacker.attack3(model,x_,y,grad)
            else:
                x_ = x_ - epsilon*grad
...
def test_CW2(testloader, model,params,config): 
    ...
    attacker = advertorch.attacks.CarliniWagnerL2Attack(lambda x: predict(model,x),params.num_classes,loss_fn=loss_fn,binary_search_steps=5, max_iterations=20)
    correct = 0
    total = 0
    for i, (x,y) in enumerate(testloader):
        x,y = x.cuda(), y.cuda()
...
if __name__=='__main__':
    ...
    optimization = 'Adam'
    params.checkpoint_dir = '%s/%s/student_adv/%s_%s_%s' %( SAVE_DIR,params.dataset, params.model, params.method,params.teacher)
    if params.exp != 'gbp':
