...
def train(trainloader, model, teacher,optimization, start_epoch, stop_epoch, params,config): 
    ...
    for epoch in range(start_epoch,stop_epoch):
        ...
        for i, (x,y) in enumerate(trainloader):
            ...
            torchvision.utils.save_image(target_mask.cpu(),'target.jpg',nrow=16)
            ...
            if params.method == 'adv':
                x_ = attacker.attack2(model,x_,y,target_mask)
            else:
                x_ = x_ - epsilon*grad
...
if __name__=='__main__':
    ...
    optimization = 'Adam'
    params.checkpoint_dir = '%s/%s/student2/%s_%s_%s' %( SAVE_DIR,params.dataset, params.model, params.method,params.teacher)
    if params.exp != 'gbp':
